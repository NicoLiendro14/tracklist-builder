...
def export_tracklist_to_console(tracklist):
    ...
    for i, entry in enumerate(tracklist, 1):
        ...
        logger.info(f"{i:02d}. [{start}] ({duration}) - {entry['track']['title']} by {entry['track']['artist']}")
...
def export_tracklist_to_text(tracklist, output_file, video_title=None):
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        ...
        for i, entry in enumerate(tracklist, 1):
            start = format_time(entry['start'])
            f.write(f"{i:02d}. [{start}] {entry['track']['title']} - {entry['track']['artist']}\n")
...
def export_tracklist_to_cue(tracklist, output_file, audio_file_path=None, video_title=None):
    """Export tracklist to a CUE file format for DJ software and audio players"""
    audio_file = os.path.basename(audio_file_path) if audio_file_path else "AUDIOFILE.mp3"
...
def export_tracklist(tracklist, output_formats=None, base_filename=None, video_title=None, video_url=None, audio_file_path=None):
...
def parse_arguments():
    parser = argparse.ArgumentParser(description="DJ Set Track Identifier - Identify tracks in a DJ mix video")
    parser.add_argument("url", help="YouTube URL of the DJ set to analyze")
    parser.add_argument("--chunk-duration", type=int, default=30, help="Duration of audio chunks in seconds (default: 30)")
    parser.add_argument("--output-dir", type=str, default="output", help="Directory to save output files (default: 'output')")
    parser.add_argument("--formats", type=str, default="txt,json,html,cue", 
